	...
	*/
	...
	function find(event){
							...
							for(var j=0;j<temp.childElementCount;j++){
								if(temp.children[j] === event.target){
									...
									if(temp.children[j].tagName === "INPUT"){
										...
										if (event.target.id === "search-content"){
											event.target.style.borderBottom="1px solid #1A73E8"
										}
									}
								}
			...
			else if(event.target.id === "search-content"){
			...
			/*else if((event.target.id === "tongle") || (event.target.id === "left-side")){
				...
				check(event.target);
				var mor=document.getElementById("moreoption");
				if(event.target.id != "moreoption"){
				var sms=document.getElementById("show-search-option");
				if(sms.style.display === "block"){
					sms.style.display="none";
					mor.style.display="inline-block";
				}
			}	
		}
}
	function check(target){
				//document.getElementById("search1").style.boxShadow="none"
				//document.getElementById("search1").style.backgroundColor="#F1F3F4";
				//document.getElementById("form").style.backgroundColor="#F1F3F4";
				//document.getElementById("input").style.backgroundColor="#F1F3F4";
						if(target.id === "header"){
							execute();
						}
					else if (target.id === "left"){
						if(target.offsetParent.id === "header"){
							execute();
						}
					}
					else if(target.id === "support" || target.id === "more-app" ||target.id === "more-app"){
						if(target.offsetParent.id === "header"){
							execute();
						}
					}
					else if(target.id === "second-half"){
						execute();
					}
	}
